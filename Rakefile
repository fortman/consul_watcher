...
spec_file = Gem::Specification.load('consul_watch_to_amqp.gemspec')

task default: :docker

task :docker_tag, [:version, :docker_image_id] do |_task, args|
  puts "#{args['docker_image_id']} => #{args['version']}"
  tag_cmd = "docker tag #{args['docker_image_id']} consul_watch_to_amqp:#{args['version']}"
...
task docker: [:build] do
  build_cmd = "docker build --build-arg gem_file=consul_watch_to_amqp-#{spec_file.version}.gem ."
